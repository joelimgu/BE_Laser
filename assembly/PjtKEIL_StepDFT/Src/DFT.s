...
start_for
	cmp r2,#64 
	beq end_for
	ldrh r4,[r0,r2,LSL #1] ; r4 = signal[i]
	mul r5,r2,r1 ; r5 = p = k*n
	; cos
	and r5,r5,#0x003F ; use only 6 bits so to make mod 64
	ldr r6,=TabCos
	ldrh r7,[r6,r5] ; r7 = TabCos[p%64]
	; end cos
	mul r7,r7,r4 ; r7 = cos*signal 
	add r3,r7 ; sum += cos*signal 
	add r2, #1
	bl start_for 
end_for
; todo res in r0
	mov r0,r7 ; débordement dans le résultat ;-( 
	pop {r7,r6,r5,r4}
	bx lr ; no read permission???? 
	endp

	
cos proc 
	;  cos(p: u32)->u16;
	and r0,r0,#0x003F ; use only 6 bits so to make mod 64
	ldr r1,=TabCos
	ldr r0,[r1,r0] ; TabCos[p%64]
	bx lr;
	endp
